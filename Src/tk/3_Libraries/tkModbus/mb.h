...
#define RS485_UART5_SWITCH_PORT GPIOB
#define RS485_UART5_SWITCH_PIN  GPIO_PIN_3



#ifdef SLAVERTU_HUART_4
#define SLAVE_HUART huart4
#define SLAVE_RS485_SWITCH_PORT RS485_UART4_SWITCH_PORT
#define SLAVE_RS485_SWITCH_PIN RS485_UART4_SWITCH_PIN
#define SLAVE_NODE_ADDRESS      0x01
#define SLAVE_BROADCAST_ADDRESS 0x00
#endif //SLAVERTU_HUART_4

#ifdef SLAVERTU_HUART_5
#define SLAVE_HUART huart5
#define SLAVE_RS485_SWITCH_PORT RS485_UART5_SWITCH_PORT
#define SLAVE_RS485_SWITCH_PIN RS485_UART5_SWITCH_PIN
#define SLAVE_NODE_ADDRESS      0x01
#define SLAVE_BROADCAST_ADDRESS 0x00
#endif //MASTERRTU_HUART_4
...
typedef struct  {
	uint8_t length;
	uint8_t msg [TX_MAX_MASTER_MSG_LENGTH];
}modbus_master_tx_msg_t;
typedef struct  {
	uint8_t length;
	uint8_t msg [RX_MAX_MASTER_MSG_LENGTH];
}modbus_master_rx_msg_t;
typedef struct  {
	uint8_t length;
	uint8_t msg [TX_MAX_SLAVE_MSG_LENGTH];
}modbus_slave_tx_msg_t;
typedef struct  {
	uint8_t length;
	uint8_t msg [RX_MAX_SLAVE_MSG_LENGTH];
}modbus_slave_rx_msg_t;
typedef struct {
	...
	modbus_exception_t *ptr_exception;			/* показчик на адресу пам'яті для повернення кода помилки, виявленої сервером					*/
	struct modbus_master_tx_msg_t *ptr_tx;		/* показчик на адресу пам'яті буфера повідомлення, що передається								*/
}sprv_t ;
#if defined (MASTERRTU_HUART_4) || defined (MASTERRTU_HUART_5)
/* Ця функція забезпечує передачу по Модбас повідомлення,сформованого користувачем
 * (повідомлення - це адреса Модбас-отримувача, інформаційні байти і CRC,
 * очікування не довше msg_timeout відповіді від сервера, і отримання відповіді серевера
 * Якщо за заданий час не отримано відповіді - результат HAL_TIMEOUT,
 * якщо отримали - результат HAL_OK.
 * Приклад використання функції - див. f_ovenSMI2_TASKS_06 (файл tk_oven_smi2)*/
...
HAL_StatusTypeDef tk4_Modbus_Session (	UART_HandleTypeDef *phuart,
 ...
 /* Якщо відповіді від сервера очікувати не потрібно - використовуйте tk4_Modbus_Tx_Session
 * */
...
При швидкості 9600 біт/сек, приблизно передається 1 байт в мс, при швидкості 19.2 кбіт/сек - 2 байти в м/с
