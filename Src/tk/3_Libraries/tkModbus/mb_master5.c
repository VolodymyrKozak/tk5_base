...
#include "tkModbus/mb.h"
#include "tkModbus/mb_master.h"
 ...
 * */
    ...
    sprv03.ptr_exception=ptr_WSR_exception;								/* задаємо посилання для отримання коду помилки від від сервера */
	sprv03.ptr_tx = &(w03.msg);
	...
	if ((mb_f_status==MODBUS_SUCCESS)&&((*sprv03.ptr_exception)==0)){						/* 12 Якщо функція виконалась без помилок, і якщо від сервера ми 	*/
		for (uint8_t i03j=0;i03j<quantity_of_registers;++i03j){			/*    отримали вміст регістрів, а не код помилки                	*/
			(*(ptr_received_values+i03j*sizeof(uint16_t)))=				/*    записуємо вміст регістрів з повідомлення від сервера 			*/
					modbus_master_rx.msg[i03j+3]*0x100 +				/*    в пам'ять за адресою, заданою прикладною програмою			*/
					modbus_master_rx.msg[i03j+4];
		}
...
LCD_Printf ("\n\n");
	...
	for (uint8_t i03i=0;i03i<RX_MAX_MASTER_MSG_LENGTH;++i03i){          /* 12 Обнуляємо буфер прийому відповіді сервера і  */
		modbus_master_rx.msg[i03i]=0x00;								/*    і переходимо в режим очікування наступної команди */
	}
	modbus_master_rx.length=0;
	master_state_5=Idle;
 ...
 * */
    ...
    sprv05.ptr_exception=ptr_WSR_exception;								/* задаємо посилання для отримання коду помилки від від сервера */
	sprv05.ptr_tx = &(w05.msg);
    //	MB_Master_Transmit ();											/* 10. Відправляємо повідомлення  */
    master_state_5 = m_msg_transmitting;
...
LCD_Printf ("\n\n");
	...
	if (slave_address==0x00){
		for (uint8_t i03i=0;i03i<RX_MAX_MASTER_MSG_LENGTH;++i03i){
			modbus_master_rx.msg[i03i]=0x00;
		}
		modbus_master_rx.length=0;
	}
 ...
 * */
    ...
    sprv06.ptr_exception=ptr_WSR_exception;								/* задаємо посилання для отримання коду помилки від від сервера				*/
	sprv06.ptr_tx = &(w06.msg);
    //	MB_Master_Transmit ();											/* 10. Відправляємо повідомлення 											*/
    master_state_5 = m_msg_transmitting;
 ...
 * */
    ...
    sprv08.ptr_exception=ptr_WSR_exception;								/* задаємо посилання для отримання коду помилки від від сервера */
	sprv08.ptr_tx = &(w08.msg);
    //	MB_Master_Transmit ();											/* 10. Відправляємо повідомлення  */
    master_state_5 = m_msg_transmitting;
	HAL_StatusTypeDef TxIt =HAL_UART_Transmit_IT(&huart4, &(w08.msg), w08.length/* RX_MAX_SLAVE_MSG_LENGTH*/);
	if (TxIt != HAL_OK) {
		mb_f08_status=MODBUS_ERR_TX;
	}
	else {
 ...
 * */
	...
	sprv10.transmitting_began_at = HAL_GetTick();							/* Засікаємо момент відправки повідомлення   */
    sprv10.ptr_exception=ptr_WSR_exception;								/* задаємо посилання для отримання коду помилки від від сервера */
	sprv10.ptr_tx = &(w10.msg);
    //	MB_Master_Transmit ();											/* 10. Відправляємо повідомлення  */
    master_state_5 = m_msg_transmitting;
...
LCD_Printf ("\n\n");
	...
	for (uint8_t i10i=0;i10i<RX_MAX_MASTER_MSG_LENGTH;++i10i){          /* 12 Обнуляємо буфер прийому відповіді сервера і  */
		modbus_master_rx.msg[i10i]=0x00;								/*    і переходимо в режим очікування наступної команди */
	}
	modbus_master_rx.length=0;
	master_state_5=Idle;
	return (mb_f_status);/* 12 Якщо mb_f_status = MODBUS_SUCCESS то:
...
static modbus_status_t MB_Master_Supervisor (sprv_t *obj) {
	...
	modbus_master_rx_msg_t *msg_ptr = NULL;
	/*
	 ...
	 *
	 * */
	while   ((HAL_GetTick() - obj->transmitting_began_at) < obj->timeout) { /* допоки не закінчиться таймаут, час очікування відповіді від сервера          */
		if (master_state_5 == m_msg_transmitting) {							/* якщо повідомлення від клієнта ще передається - чекаємо*/
			/*do nothing*/
		}
		else if (master_state_5 == m_msg_transmitting_ERROR){               /*  якщо отримане переривання про помилку при передачі, 					*/
			MB_sv_status=MODBUS_ERR_TX;										/*      фіксуємо код помилки, виходимо з очікування 						*/
			master_state_5=Idle;											/*      модбас поверстається у стан готовності до передачі наступних функцій*/
			break;
		}
		else if (master_state_5 ==m_msg_transmitting_Cplt){					/*  якщо отримано повідомлення про успішне завершення передачі				*/
			  	  if (obj->waited_slave_response_address==0x00){			/*    якщо це циркулярне повідомлення 										*/
			  		MB_sv_status = MODBUS_SUCCESS;							/*  фіксуємо код успішного завершення, виходимо з очікування 				*/
			  			master_state_5=Idle; 								/*  модбас повертається у стан готовності до передачі наступних функцій	*/
			  			break;
			  	  }

																			/*      запускаємо UART  режим прийому повідомлення - відповіді від сервера */
			  volatile  HAL_StatusTypeDef RxIt =HAL_UART_Receive_IT(&huart4, &(modbus_master_rx.msg), RX_MAX_MASTER_MSG_LENGTH);
			  if (RxIt != HAL_OK){											/*  якщо зафіксована помилка прийому */
				  MB_sv_status = MODBUS_ERR_RX;								/*  фіксуємо код помилки, виходимо з очікування 							*/
				  master_state_5=Idle; 										/*  модбас поверстається у стан готовності до передачі наступних функцій	*/
				  break;
			  }
			  else {
				   master_state_5=Waiting_for_reply;						/*  якщо помилки не зафіксовано, продовжуємо прийом */
			  }

		}

		else if (master_state_5==Waiting_for_reply){						/* чекаємо, поки не буде отримане повідомлення */
			/* do nothing*/
		}
		else if (master_state_5==m_msg_recived_RxIdle){									/* якщо отримано переривання про завершення прийому повідомлення */


			uint16_t crc_calc_rx=0;
			if (modbus_master_rx.length == obj->waited_normal_response_length){			/* якщо довжина повідомлення - це довжина нормальної очікуваної відповіді */
				crc_calc_rx = CRC_16x (&(modbus_master_rx.msg), 			    		/* 2. Обчислюємо 16-бітний CRC   									*/
						modbus_master_rx.length -2*(sizeof(uint8_t)));

				if (modbus_master_rx.msg[0] != obj->waited_slave_response_address){		/* якщо це відповідь від сервера, від якого ми відповідь і очікували*/
					  MB_sv_status = MODBUS_ERR_SLAVE_ADDR;
					  master_state_5=Idle;
					  break;
				}
				if (modbus_master_rx.msg[1] != obj->waited_slave_response_function){          /*якщо це нормальна відповідь на наш запит */
					  MB_sv_status = MODBUS_ERR_FC;
					  master_state_5=Idle;
					  break;
				}
				if (
						(modbus_master_rx.msg[modbus_master_rx.length-1] != crc_calc_rx / 0x100) ||/* якщо CRC у повідомленні правильний*/
						(modbus_master_rx.msg[modbus_master_rx.length-2] != crc_calc_rx % 0x100)
						){
					  MB_sv_status = MODBUS_ERR_CRC;
					  master_state_5=Idle;
					  break;
				}
				/* це нормальне повідомлення*/
				msg_ptr=obj->ptr_tx;
				if (obj->waited_slave_response_function==READ_HOLDING_REGISTERS){
									/*викликаємо функцію обробки нормального повідомлення, у випадку команди 0x03,можна  так обійтися */
					/* Byte_Count _test: */
					if ((modbus_master_rx.msg[2]!=
							((msg_ptr->msg[0])*2)) ||
							(modbus_master_rx.length != ((msg_ptr->msg[0])*2)+5)){
						MB_sv_status=MODBUS_ERR_fromSLAVE;
					}
					if (MB_sv_status == MODBUS_ERR_fromSLAVE){
						master_state_5=Idle;
						break;
					}
					else {
				...
				if (obj->waited_slave_response_function==WRITE_SINGLE_COIL){
					/*викликаємо функцію обробки нормального повідомлення, у випадку команди 0x03,можна  так обійтися */
					for (uint8_t i03=0;i03<modbus_master_rx.msg[modbus_master_rx.length-1]; i03++){
							if (modbus_master_rx.msg[i03] == msg_ptr->msg[i03]) {
								 MB_sv_status = MODBUS_ERR_fromSLAVE;
							}
					}
					if (MB_sv_status == MODBUS_ERR_fromSLAVE){
				...
				if (obj->waited_slave_response_function==DIAGNOSTIC ){
					if (MB_sv_status == MODBUS_ERR_fromSLAVE){
							master_state_5=Idle;
							break;
					}
			...
			else if (modbus_master_rx.length==obj->waited_error_response_length){
				crc_calc_rx = CRC_16x (&(modbus_master_rx.msg), 			    			/* 2. Обчислюємо 16-бітний CRC   									*/
							modbus_master_rx.length -2*(sizeof(uint8_t)));

				if (modbus_master_rx.msg[0] != obj->waited_slave_response_address){
						  MB_sv_status = MODBUS_ERR_SLAVE_ADDR;
						  master_state_5=Idle;
						  break;
				}
				else if (modbus_master_rx.msg[1] != (obj->waited_slave_response_function |= 0x08)){
						  MB_sv_status = MODBUS_ERR_FC;
						  master_state_5=Idle;
						  break;
				}
				else if (
							(modbus_master_rx.msg[modbus_master_rx.length-1] != crc_calc_rx / 0x100) ||
							(modbus_master_rx.msg[modbus_master_rx.length-2] != crc_calc_rx % 0x100)
							){
						  MB_sv_status = MODBUS_ERR_CRC;
						  master_state_5=Idle;
						...
						*(obj->ptr_exception)=modbus_master_rx.msg[3];
				}
			}
			else { /**/
		...
		} //end of (master_state_5==m_msg_recived_RxIdle){

		else {}
	}	// while
    if  (master_state_5 == m_msg_transmitting) {
    	MB_sv_status = MODBUS_ERR_TIMEOUT;
    	master_state_5=Idle;
